...
| Bench baseline                               | `make bench-compare` fails on any alloc regression or >5% wall-time regression. Local baseline lives at `.bench-baseline/main.txt` (`.gitignore`d — refreshed via `make bench-baseline`); CI regenerates from `main` on the runner before comparing against PR head, giving same-hardware comparison. The published perf snapshot lives at [`docs/performance/go.md`](../../performance/go.md). |
| Cross-format consistency                     | `AssertCrossFormatConsistency` checks decode parity vs `encoding/json` for every fixture. Types that aren't JSON-roundtrippable (e.g. `map<bool, V>` — `encoding/json` rejects non-string keys) set `Spec.SkipJSONComparisons: true`, which suppresses both the comparison subtests and the JSON-baseline benches. (Without skipping the bench, `json.Marshal` returns empty + error for such types and the benchmarks would time the error path — producing misleading sub-100 ns/op numbers that suggest JSON outperforms the codec when in fact JSON is doing no real work.) |
...
- [`docs/performance/go.md`](../../performance/go.md) — published perf snapshot (per-fixture marshal/unmarshal numbers, alloc budgets, Codec-vs-JSON ratios)
